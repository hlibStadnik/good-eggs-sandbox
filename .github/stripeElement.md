...
Ensure you’re on the latest Stripe.js version by including the following script tag `<script src=“https://js.stripe.com/clover/stripe.js”></script>`. Learn more about [Stripe.js versioning](https://docs.stripe.com/sdks/stripejs-versioning.md).
...
func handlePaymentSheet(w http.ResponseWriter, r *http.Request) {
  ...
  csParam.AddExtra("components[mobile_payment_element][enabled]", true)
  csParam.AddExtra(
    "components[mobile_payment_element][features][payment_method_save]",
    "enabled",
  )
  ...
  writeJSON(w, struct {
    ...
    CustomerSessionClientKey   string `json:"customerSessionClientSecret"`
    Customer       string `json:"customer"`
    PublishableKey string `json:"publishableKey"`
  }{
    PaymentIntent: pi.ClientSecret,
    CustomerSessionClientKey: cs.ClientSecret,
    Customer: c.ID,
    PublishableKey: "<<YOUR_PUBLISHABLE_KEY>>",
  })
}
...
// Make async request to your own server
    MyBackend.getPaymentConfig(new NetworkCallback() {
      ...
      public void onSuccess(NetworkResult networkResult) {
        if (networkResult.isSuccess()) {
          customerConfig = new PaymentSheet.CustomerConfiguration.createWithCustomerSession(
            networkResult.getCustomer(),
            networkResult.getCustomerSessionClientSecret()
